...
export async function main(ns) {
    const serverName = ns.args[0] || 'n00dles';

    // If it looks like the server is NUKEable, do it!
    if ( canNuke(ns, serverName) ) {
        ns.nuke(serverName);
        var message = ns.hasRootAccess(serverName) ? "Successful NUKE" : "Failed to NUKE";
        ns.tprint(message);
    // A number of ports need to be opened before NUKE can be used.
        ...
        // Try to open ports.
        ns.tprint(totalPortsNeeded+" port(s) must be open to NUKE this server.");
        var p = 0;
